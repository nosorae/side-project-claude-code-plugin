...
`product-blueprint.html`을 외부와 공유 가능한 GitHub Pages URL로 노출할 수 있습니다. **메인 코드는 private 그대로 유지**되고 블루프린트만 공개됩니다.

`/init-project` 실행 중 묻습니다:
```
블루프린트를 공개 URL로 공유하시겠어요? (y/N)
```

`y` 선택 시 자동으로:
- public sync repo 생성: `{프로젝트}-blueprint`
- SSH Deploy Key 생성 → sync repo에 등록 (write 권한)
- private key를 원본 repo의 `BLUEPRINT_DEPLOY_KEY` secret으로 저장 (로컬 키는 즉시 삭제)
- `.github/workflows/sync-blueprint.yml` 추가 → `product-blueprint.html` 변경 시 자동 미러링
- GitHub Pages 활성화

**결과 URL**: `https://{owner}.github.io/{프로젝트}-blueprint/`

**왜 Deploy Key 방식?**
- PAT 불필요 (사용자가 web에서 토큰 발급할 일 없음)
- 권한이 sync repo에만 한정 → private repo의 다른 코드 누수 0
- Free 계정에서도 동작 (public repo의 Pages는 무료)

나중에 켜고 싶으면 `/init-project`를 다시 실행하거나 같은 단계를 수동으로 따라할 수 있습니다.
...
- (옵션) 블루프린트 공개 sync repo + GitHub Pages
