...
**Claude가 하는 일:**
팀메이트 3명이 각각 독립 세션에서 조사합니다.
- 사용자 관점: 사용자가 진짜 원하는 게 뭔지
- 비즈니스 관점: 수익 모델은 뭐가 맞는지
- 기술 관점: AI 코딩으로 만들 수 있는지

각자 웹 검색까지 해서 조사한 다음, 결과를 놓고 토론합니다. 한쪽으로 치우치지 않게 세 관점을 맞춰보는 겁니다. 핵심 가치, MVP 범위, 유저 플로우를 정하고 기획서(PRD)를 씁니다.
...
| 기획 | 방향을 판단하고 수정한다 | 3관점 팀메이트 토론으로 PRD를 작성한다 |
