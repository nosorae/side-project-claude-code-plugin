...
1. **최신 스킬 실시간 검색** (정적 목록 사용 금지 — 시점에 따라 최고의 스킬이 다르므로 반드시 실시간 검색)
   1. Claude Code 마켓플레이스에서 해당 플랫폼 키워드로 검색
   2. GitHub에서 `"claude code skill {플랫폼}"`, `"claude plugin {플랫폼}"` 검색 → 스타 수 기준 상위 결과 확인
   3. 웹 검색으로 `"best claude code {플랫폼} plugin 2026"` 최신 추천 확인
   4. **위 모두에서 못 찾으면 — 커스텀 스킬 자동 생성 (필수)**:
      - 해당 플랫폼의 공식 문서, 공식 가이드라인을 웹 검색
      - 조사 결과를 기반으로 `.claude/skills/{플랫폼}-best-practices/SKILL.md` 자동 생성
      - 최소 포함: 프로젝트 구조 컨벤션, 코딩 스타일, 테스트 패턴, 성능 주의사항

2. **검색 결과를 사용자에게 보여주고 선택받는다**
   ```
   [{플랫폼}] 베스트 프랙티스 스킬 검색 결과:
   1. [스킬명] (★ N) — 핵심 기능 요약
   2. [스킬명] (★ N) — 핵심 기능 요약
   3. 커스텀 생성 (공식 문서 기반)
