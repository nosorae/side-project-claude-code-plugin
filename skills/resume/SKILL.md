...
name: resume
description: |
  새 세션 시작 시 프로젝트 최신 상태를 파악하고 이어서 작업할 수 있게 하는 스킬. 핸드오프 문서, Git 히스토리, GitHub Issues, SSOT 문서를 종합 분석한다.
  이 스킬은 다음과 같은 요청에 반드시 사용한다: "이어서 해줘", "프로젝트 상태 파악", "어디까지 했지", "지난 작업 확인", "/resume", "새 세션 시작".
  새 세션에서 이전 작업을 이어받아야 하는 맥락이면 이 스킬을 사용한다.
user_invocable: true
---
