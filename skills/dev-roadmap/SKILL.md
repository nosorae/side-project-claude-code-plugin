...
1. 작업을 릴리스 버전 기반 마일스톤으로 분류한다:
   - **v0.1.0**: MVP — 핵심 가치를 전달하는 최소 기능 + 프로젝트 셋업
   - **v0.2.0**: 보조 기능 — MVP 완성에 필요한 나머지 기능
   - **v1.0.0**: 정식 릴리스 — 테스트, 버그 수정, 배포 마무리
   - 버전은 프로젝트 규모에 따라 조정 가능 (v0.3.0 등 추가 가능)
...
> **작성 원칙**: 어떤 LLM이 이 작업 설명만 보고도 동일하게 구현할 수 있어야 한다. "설정해라"가 아니라 "이 값으로 이 파일에 설정해라". 구체적인 파일 경로, 설정값, 검증 명령어를 반드시 포함한다. 특정 기술을 선택한 이유와 대안 대비 트레이드오프도 적어서, 나중에 "왜 이렇게 했지?"에 답할 수 있게 한다.
...
> **작성 원칙**: 개발 경험이 없는 사람도 따라할 수 있어야 한다. 어디를 클릭하고 뭘 입력하는지 스텝바이스텝으로. 선택지가 있으면 추천과 이유를 같이. 이 서비스를 고른 이유와 대안의 장단점도 적어서, 나중에 다른 선택을 할 수 있는 근거를 남긴다. 흔한 실수와 주의사항도 포함한다.
