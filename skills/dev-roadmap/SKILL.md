...
> **작성 원칙**: 어떤 LLM이 이 작업 설명만 보고도 동일하게 구현할 수 있어야 한다. "설정해라"가 아니라 "이 값으로 이 파일에 설정해라". 구체적인 파일 경로, 설정값, 검증 명령어를 반드시 포함한다.
...
> **작성 원칙**: 개발 경험이 없는 사람도 따라할 수 있어야 한다. 어디를 클릭하고 뭘 입력하는지 스텝바이스텝으로. 선택지가 있으면 추천과 이유를 같이. 흔한 실수와 주의사항도 포함한다.
