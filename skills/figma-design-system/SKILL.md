...
description: "Use when the user wants to generate a mobile design system from a PRD or spec markdown file and export it to Figma. Triggered by requests like 'PRD로 디자인 시스템 만들어줘', 'Figma 디자인 시스템 생성', '/figma-design-system'."
---
...
PRD/스펙 마크다운 파일을 분석하여 디자인 시스템 HTML을 생성하고, Figma MCP `generate_figma_design`으로 Figma 파일에 내보낸다.

## 사용법

```bash
# 기본: PRD 경로 + Figma URL
/figma-design-system docs/plans/my-app-spec.md https://www.figma.com/design/ABC123/MyApp

# Figma URL 없이 (실행 중 질문)
/figma-design-system docs/plans/my-app-spec.md
```
...
- `Write` 도구로 `.claude/skills/prd-to-figma/tokens.css`에 저장
...
- `Write` 도구로 `.claude/skills/prd-to-figma/_temp/design-system.html`에 저장
...
- 서버 URL: `http://localhost:8765/design-system.html`
...
1. **생성된 파일 목록**:
   - tokens.css 경로: `.claude/skills/prd-to-figma/tokens.css`
   - design-system.html 경로: `.claude/skills/prd-to-figma/_temp/design-system.html`
