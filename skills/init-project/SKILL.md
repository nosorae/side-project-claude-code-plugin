...
| 플러그인 설치 실패 (네트워크 오류) | 로컬 경로(`~/side-project-claude-settings`)에서 파일 직접 복사로 폴백 |
| 로컬 레포도 없음 | `git clone https://github.com/nosorae/side-project-claude-settings.git ~/side-project-claude-settings` 후 복사 |
...
포함된 항목:
- 플러그인: side-project-claude-settings (규칙 7개 + 스킬 15개 자동 적용)
