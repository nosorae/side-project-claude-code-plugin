...
- 배경: `#F8F9FA` (시스템 전체 배경)
- **피드백 모드 스크립트 삽입** (필수): `</body>` 직전에 `<script src="${CLAUDE_PLUGIN_ROOT}/assets/feedback-mode.js"></script>` 삽입. 플러그인 경로를 알 수 없으면 `feedback-mode.js`를 HTML과 같은 디렉토리에 복사한 뒤 `<script src="feedback-mode.js"></script>`로 삽입한다.
